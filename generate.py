...
def data_readiness_html(update_time, market_ctx=None, report_phase=None):
    market_ctx = market_ctx or {}
    report_phase = report_phase or {'label': '資料更新', 'note': '請以頁面顯示的資料時間為準。'}
    temp = market_ctx.get('temperature', '中性')
    regime = market_ctx.get('regime', '資料不足')
    return (
        f'<section class="sc data-check-card" id="data-check">'
        f'<div class="st">資料檢查</div>'
        f'<div class="method-lead">本頁是靜態市場報告，數字代表這次產生報告時抓到的公開資料。</div>'
        ...
        f'<div><span>資料範圍</span><b>公開市場資料</b><small>用來理解市場狀態、標的體質與資料限制，不取代券商報價與個人判斷。</small></div>'
...
#data-mode .data-check-card{border-radius:8px}
...
body.target-detail-open #data-mode>.data-check-card{display:none}
...
def build_html(idx_html, tw_s, tw_e, us_s, us_e, bonds, update_time, market_ctx=None, report_phase=None):
    ...
    return (
        ...
        f'</section>\n'
        f'{methodology_html()}\n'
        f'{data_readiness_html(update_time, market_ctx, report_phase)}\n'
        f'</section>\n'
        f'</div></main>\n'
        f'</div>\n'
